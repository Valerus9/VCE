...
namespace economy_rgo {
	...
	void update_rgo_employment(sys::state& state) {
		province::for_each_land_province(state, [&](dcon::province_id p) {
			...
			static std::vector<dcon::commodity_id, common_types::cache_aligned_allocator<dcon::commodity_id>> ordered_rgo_goods;
	...
	float rgo_efficiency(sys::state& state, dcon::nation_id n, dcon::province_id p, dcon::commodity_id c) {
		...
		return std::max(0.f, 1.0f + nmod + pmod + specific_pmod + state.defines.vce_base_rgo_efficiency_bonus);
	}
