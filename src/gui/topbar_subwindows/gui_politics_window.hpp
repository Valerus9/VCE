	...
	class politics_choose_party_button : public button_element_base {
		...
		void update_tooltip(sys::state& state, int32_t x, int32_t y, text::columnar_layout& contents) noexcept override {
			...
			if(state.world.political_party_get_trigger(party)) {
				text::add_line(state, contents, "vce_political_party_trigger", text::variable_type::date_long_0, state.world.political_party_get_start_date(party),
				...
				text::add_line(state, contents, "vce_political_party_no_trigger", text::variable_type::date_long_0, state.world.political_party_get_start_date(party),
				text::variable_type::date_long_1, state.world.political_party_get_end_date(party));
			}
		}
	};
